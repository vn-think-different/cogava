...
import { NhanVien, VaiTroNhanVien } from '../types';
...
export const EmployeeManagementView: React.FC = () => {
  const { employees, addEmployee, updateEmployee, toggleEmployeeStatus, currentUser } = useApp();
  ...
  const chinhCount = employees.filter(e => e.vaiTro === 'CHINH' && e.trangThai === 'DANG_LAM').length;
  const phuCount = employees.filter(e => e.vaiTro === 'PHU' && e.trangThai === 'DANG_LAM').length;
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-stone-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-stone-500 mt-0.5">
              Quản lý danh sách, vai trò lương chính/phụ và tài khoản ngân hàng chuyển lương
            </p>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-stone-100 flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-stone-600">Hiện có:</span>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-stone-200 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-stone-100">
              <h2 className="text-base font-bold text-stone-900">
                {editingEmp ? `Chỉnh sửa nhân viên: ${editingEmp.hoTen}` : 'Thêm nhân viên bắt gà mới'}
              </h2>
              <button
